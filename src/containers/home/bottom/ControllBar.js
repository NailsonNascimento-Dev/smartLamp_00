...
class HomeBottomView extends Component {
  ...
  getData() {
    const {
      dpFun: { isSupportWhite, isSupportScene },
    } = Config;
    ...
    return [
      ...
      console.log(' impressão do data.key: ' + data.key);
      console.log('impressão da função power: ' + power);
      return {
        ...commonProps,
        ...data,
  ...
  handlePress = workMode => {
    ...
    if (workMode === TIMER_KEY) {
      ...
      console.log('Power code ************* ' + powerCode);
